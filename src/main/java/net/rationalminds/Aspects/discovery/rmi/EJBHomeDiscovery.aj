...
public abstract aspect EJBHomeDiscovery extends JSnooperAspectCore {

	private static final Class ejbRemoteClass = findClassIfPresent(
			"javax.ejb.EJBObject", EJBHomeDiscovery.class.getClassLoader());
	private static final Class ejbHomeClass = findClassIfPresent(
			"javax.ejb.EJBHome", EJBHomeDiscovery.class.getClassLoader());

	public pointcut remoteExecution() : within(Remote+) && execution(public * *(..) throws RemoteException);

	public pointcut ejbHomeMethod() : within(javax.ejb.EJBHome+) && remoteExecution();
	 ...
	 * @param ejb
	 */
	after(Remote ejb)returning(Object returnObject) : ejbHomeMethod() && this(ejb) {
		String methodName = thisJoinPointStaticPart.getSignature().getName();
		if (methodName == null || ignoreMethods.contains(methodName)
				|| methodName.startsWith("__WL")) {
	 ...
	 * Exception Handling
	 */
	after(Object ejb)throwing(Throwable e):ejbHomeMethod()&& this(ejb){
		String methodName = thisJoinPointStaticPart.getSignature().getName();
		if (methodName == null || ignoreMethods.contains(methodName)
				|| methodName.startsWith("__WL")) {
	 ...
	 * @param e
	 */
	/*before(Exception e):exceptionHandler(e) && cflow(ejbHomeMethod()){
		if (e == null) {
