package net.rationalminds.Aspects.discovery.outgoing.apachehttp4;
...
public abstract aspect ApacheHttp4 extends JSnooperAspectCore {
	...
	before() : pointCut() && if(CONSTANTS.CONTROL.PROBE_ON)
	{
		...
		if(allArgs != null && allArgs.length > 0 )
		{
			...
			{
				if(ReflectionUtils.instanceOf("org.apache.http.HttpMessage", allArgs[i]) && allArgs[i] != null)
				{
					if(ReflectionUtils.invokeMethodObject("getFirstHeader", allArgs[i], new Object[]{CONSTANTS.GUID}) == null)
					{
						String guid = getThreadLocal().get().getGuid();
						ReflectionUtils.invokeMethod("setHeader", allArgs[i], new Object[]{CONSTANTS.GUID, guid});
						break;
					}
				}
			}
